...
`.trim()
      ...
      const result = await dockerManager.buildImage(dockerfile, '/tmp/claude', scriptPath)
    ...
    it('should fail to build invalid Dockerfile', async () => {
      ...
      const result = await dockerManager.buildImage(invalidDockerfile, '/tmp/claude', scriptPath)
...
`.trim()
      ...
      const buildResult = await dockerManager.buildImage(dockerfile, '/tmp/claude', '/bin/echo')

      if (buildResult.success) {
        const runResult = await dockerManager.runContainer(buildResult.imageId, [])
  ...
  describe('cleanup', () => {
    it('should cleanup Docker image', async () => {
      ...
      if (buildResult.success) {
        // Should not throw
        await expect(dockerManager.cleanup(buildResult.imageId)).resolves.not.toThrow()
      }
    }, 30000)

    it('should handle cleanup of non-existent image gracefully', async () => {
      // Should not throw, just warn
      await expect(dockerManager.cleanup('non-existent-image')).resolves.not.toThrow()
    })
  })
})
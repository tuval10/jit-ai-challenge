...
- Implement the challenge within your own forked repo.
- The Python scripts are provided in the repository link

### Goal:

You aim to write a tool that uses generative AI to wrap existing Python scripts with a Dockerfile. The tool will use the API OpenAI, and will need to:
...
1. **Integrate with an AI Model**:
    - Write the tool to interact with an AI model using an API key. The tool needs to do the following:
        - The code should accept the provided Python script (and any tool with the same type of input and output) and generate a Dockerfile that wraps the script.

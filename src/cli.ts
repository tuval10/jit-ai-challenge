...
program
  ...
  .description(
    'AI-powered tool to automatically generate Dockerfiles for scripts'
  )
  .version('1.0.0');
...
program
  ...
  .option(
    '-k, --api-key <key>',
    'API key for the LLM provider (or set environment variable)'
  )
  .option(
    '-s, --skip-test',
    'Skip validation tests (tests run by default)',
    process.env.DEFAULT_SKIP_TEST_MODE === 'true'
  )
  .option(
    '-c, --cleanup',
    'Cleanup Docker image after completion',
    process.env.DEFAULT_CLEANUP_MODE === 'true'
  )
  .option(
    '--temperature <temp>',
  ...
  .action(async (scriptPath: string, readmePath: string, options) => {
    try {
      ...
      if (!apiKey) {
        console.error(
          `❌ API key required. Provide via --api-key or set ${provider.toUpperCase()}_API_KEY environment variable`
        );
        process.exit(1);
      }
      ...
      console.log(
        `🤖 Using ${provider} with model: ${LLMProviderFactory.getDefaultModel(
          provider
        )}`
      );
      ...
      if (errorMessage.includes('API key')) {
        ...
        console.error(
          '💡 Hint: Check that the script and README files exist and are readable'
        );
      }
...
Examples:
  ...
  $ script-dockerizer word_reverser.py README_word_reverser.md --cleanup
  $ script-dockerizer vowel_counter.js README_vowel_counter.md --provider anthropic --skip-test
